...
**hs-fork v3.2.2-hs.1** — This orchestrator depends on the home-still MCP server. Before launching Stage 1, the pipeline runs a health preflight; integrity gates at Stage 2.5 and Stage 4.5 use home-still catalog/paper_get to verify every citation. See Tool Routing Policy below.

---

## Tool Routing Policy — home-still MCP (REQUIRED)

This pipeline hard-depends on `home-still`. It does not fetch papers itself — its downstream skills (`deep-research`, `academic-paper`, `academic-paper-reviewer`) do — but the orchestrator enforces the policy across them:

### Stage 0: Pipeline preflight (before Stage 1)

Before any stage launches, the orchestrator calls:
1. `mcp__home-still__system_status` — pipeline health, document counts.
2. `mcp__home-still__scribe_health` — VLM slot availability (blocks `paper_download` if unhealthy).
3. `mcp__home-still__distill_status` — confirm `compute_device: "Cuda"`.

If any fails, halt and present the user with two choices:
- (A) Fix home-still (surface error details) and re-run preflight.
- (B) Proceed in **degraded mode** — web-only, no catalog, no semantic search. Explicitly flag that integrity gates will be weaker and that citation verification degrades to DOI resolver + WebSearch only.

Record the preflight result in the Material Passport (pipeline state).

### Stage 2.5 — Pre-review integrity check (home-still cascade)

For every reference in the bibliography:
1. `mcp__home-still__catalog_read` by DOI → if `HS_VERIFIED`, recorded.
2. `mcp__home-still__paper_get` by DOI → if resolved + title match, `VERIFIED`.
3. If neither: `UNVERIFIED`. Block the transition to Stage 3 (peer review) until the author resolves it.

This replaces the original pipeline's DOI-resolver + Semantic-Scholar-API cascade.

### Stage 4.5 — Final integrity check (same cascade, stricter)

Same as Stage 2.5, but:
- Any `UNVERIFIED` entry at this stage is a **critical failure** — the pipeline cannot finalize the manuscript.
- Additionally, every `HS_VERIFIED` source must still exist (re-run `catalog_read` to catch any that may have been purged via `distill_purge` between stages).

### Handoff to downstream skills

When dispatching `deep-research` (Stage 1) or `academic-paper` (Stage 2, 4), pass forward the Material Passport with:
- `preflight_ok: true/false` (so downstream skills know the policy applies)
- `degraded_mode: true/false` (if user chose to proceed without home-still)

Downstream skills MUST check `preflight_ok` before running; if false and `degraded_mode` is true, they may run in web-only mode with loud warnings.
