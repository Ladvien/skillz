...
This agent is the gate. All reference verification runs through the home-still cascade (catalog_read → paper_get → WebSearch). See the parent `SKILL.md` "Tool Routing Policy" and Phase A0 / A1 below for the exact tool sequence. If home-still is unreachable and the pipeline is in `degraded_mode`, this agent runs WebSearch-only with a loud disclaimer in the verification report.
...
#### A0. home-still Catalog Batch Verification (primary, fastest)

Run this first for ALL references. Routing:

| home-still Result | Action |
|-------------------|--------|
| `HS_VERIFIED` (stem found in catalog, title+year match ≥0.70 Levenshtein) | Skip to A2 (bibliographic accuracy) — reference is physically in our library, existence is proven |
| `HS_NOT_FOUND` | Proceed to A1 (home-still `paper_get` multi-provider lookup) |
| `HS_DOI_MISMATCH` (DOI resolves in catalog but title diverges) | Flag as SERIOUS — possible DOI Misdirection (Compound Deception Pattern #5) |
| `HS_UNAVAILABLE` (MCP unreachable and `degraded_mode: true`) | Skip A0 + A1, fall through to A1b (WebSearch-only). Record `[HS-UNAVAILABLE]` in the audit trail and note the degraded verification mode in the final report |

Procedure:
```
For each reference r:
  if r.doi:
    result = mcp__home-still__catalog_read(doi=r.doi)
  elif r.stem:
    result = mcp__home-still__catalog_read(stem=r.stem)
  else:
    skip to A1 (no local key)

  if result.matches (title Levenshtein >= 0.70, year +/-1):
    r.status = HS_VERIFIED
    r.hs_stem = result.stem
  elif result.doi resolved but title Levenshtein < 0.70:
    r.status = HS_DOI_MISMATCH (SERIOUS)
  else:
    r.status = HS_NOT_FOUND  # fall through
```

Record every call in the audit trail: DOI / stem queried, result, title-match score.

#### A1. home-still Multi-Provider Lookup (fallback)

For references that returned `HS_NOT_FOUND` in A0:

```
For each such reference r:
  result = mcp__home-still__paper_get(doi=r.doi)   # hits arXiv, OpenAlex, S2, Europe PMC, CrossRef, CORE
  if result.matches (title Levenshtein >= 0.70, year +/-1):
    r.status = VERIFIED
    r.provider = result.provider
    # Optionally: mcp__home-still__paper_download(doi=r.doi) to grow the library
  elif r.doi resolves but title Levenshtein < 0.70:
    r.status = DOI_MISMATCH (SERIOUS)
  else:
    # Fall through to A1b
    proceed_to_A1b()
```

#### A1b. WebSearch Existence Check (final fallback, catches non-indexed sources)

For references still not verified after A0 + A1:
```
...
Determination:
- PLAUSIBLE: WebSearch confirms publisher page, matching details — not in our providers but real
- NOT_FOUND: Cannot find any match after 3 different search queries — suspected fabrication → MUST be flagged as SERIOUS issue
- MISMATCH: Found a similar but different publication — suspected hallucinated mashup → MUST be flagged as SERIOUS issue with correct details provided

⚠️ CRITICAL: There is NO "uncertain" or "difficult to verify" category. If you cannot positively verify a reference exists with its exact bibliographic details after all three tiers (A0 + A1 + A1b), it is NOT_FOUND or MISMATCH. Both require correction.
```

**Summary:** HS_VERIFIED > VERIFIED > PLAUSIBLE > (NOT_FOUND / MISMATCH / DOI_MISMATCH). The pipeline cannot proceed past Stage 2.5 until every reference is at PLAUSIBLE or better. The pipeline cannot finalize past Stage 4.5 until every reference is at VERIFIED or HS_VERIFIED (PLAUSIBLE is allowed at Stage 4.5 only with explicit user acknowledgment of the limitation).
