...
All recurring discovery, retraction checks, and keyword-evolution scans go through `mcp__home-still__*` tools. See the parent `SKILL.md` "Tool Routing Policy". External alert platforms (Google Scholar, PubMed, Retraction Watch) are *complementary* channels for the user — not replacements for the home-still primary source.

### home-still monitoring primitives

- `mcp__home-still__catalog_recent` — surface papers added to the library since the last digest.
- `mcp__home-still__distill_search` — re-run the original research question's semantic queries against the growing corpus; new top-k results not seen before are candidates for the digest.
- `mcp__home-still__paper_search` — sweep the 6 web databases for new DOIs matching the tracked keywords and authors. For each hit not already in the catalog, optionally `mcp__home-still__paper_download` to ingest.
- `mcp__home-still__catalog_read` — for each source in the bibliography, pull current metadata; compare against last-digest snapshot to detect retractions/corrections flagged upstream.
- `mcp__home-still__paper_get` — resolve a DOI to its current metadata across providers; used to catch retractions that are in CrossRef but not yet in the local corpus.
