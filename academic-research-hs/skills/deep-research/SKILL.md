...
**hs-fork v2.8.1-hs.1** — This skill is hard-wired to the home-still MCP server (see Tool Routing Policy below). Every sub-agent MUST use `mcp__home-still__*` tools for paper discovery, reading, and citation. Web APIs and generic search are fallbacks only.

---

## Tool Routing Policy — home-still MCP (REQUIRED)

This skill depends on the **home-still** MCP server for all paper discovery, reading, and citation. Every sub-agent MUST follow this policy, overriding any web-search defaults inherited from upstream documentation.

### Discovery — ordered, try each in order

1. `mcp__home-still__distill_search` — semantic search over the indexed personal corpus (BGE-M3, Qdrant). Try first for every literature query.
2. `mcp__home-still__catalog_list` / `mcp__home-still__catalog_recent` — browse library metadata when the query is broad or temporal.
3. `mcp__home-still__paper_search` — multi-database web search (arXiv, OpenAlex, Semantic Scholar, Europe PMC, CrossRef, CORE) for gaps.
4. Only if 1–3 fail to locate a known paper: generic web search / external APIs.

### Ingest — anything worth keeping from web discovery

1. `mcp__home-still__paper_download` with DOI preferred, URL fallback. Runs on the home-still host; auto-triggers scribe conversion and distill indexing.
2. Poll `mcp__home-still__distill_status` / `mcp__home-still__distill_exists` until indexed (usually minutes).
3. Resume downstream work using the new stem via `catalog_read` / `markdown_read`.

### Reading

- `mcp__home-still__markdown_read` — cleaned, layout-corrected markdown.
- `mcp__home-still__catalog_read` — metadata (DOI, authors, venue, year, abstract).

Never re-fetch a PDF when the stem exists in the catalog.

### Citations & integrity (non-negotiable)

- All bibliographic data must trace to a real `catalog_read` entry or a verified `mcp__home-still__paper_get` result.
- If a DOI cannot be resolved, do not cite it. No fabricated references.
- Source Verification Agent (see `agents/source_verification_agent.md`) validates every citation against the catalog before the Report Compiler runs.

### Health preflight — before any long-running mode (full, systematic-review, lit-review, monitoring)

1. `mcp__home-still__system_status` — pipeline counts, service health.
2. `mcp__home-still__scribe_health` — VLM slot availability for conversions.
3. `mcp__home-still__distill_status` — confirm `compute_device: "Cuda"` before batch ingest; warn if CPU.

### Failure modes

- If home-still MCP is unreachable: halt and ask the user whether to proceed with web-only fallback. Flag this explicitly — integrity gates and Source Verification may fail in this mode.
- If a paper is downloaded but `distill_exists` returns false after ~5 minutes: proceed with `markdown_read` directly; embed can backfill later.
- If `distill_status` reports CPU device: warn the user that large batch searches will be slow; recommend running on the CUDA host.

### Artifact placement

- Papers (PDF + markdown) stay on the home-still host. Do not copy them locally.
- Reports, bibliographies, synthesis narratives → `~/research/<topic-slug>/` on the local machine.
