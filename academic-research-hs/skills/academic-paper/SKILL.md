...
**hs-fork v3.0.2-hs.1** — This skill is hard-wired to the home-still MCP server (see Tool Routing Policy below). Every sub-agent that pulls references or verifies citations MUST use `mcp__home-still__*` tools. Past-papers for Style Calibration may come from the home-still library via `mcp__home-still__catalog_read` + `markdown_read` on stems the user names.

---

## Tool Routing Policy — home-still MCP (REQUIRED)

This skill depends on the **home-still** MCP server for all paper discovery, reading, and citation. Every sub-agent that touches the bibliography MUST follow this policy, overriding any web-search defaults inherited from upstream documentation.

### Literature for lit-review sections

1. `mcp__home-still__distill_search` — semantic search over the indexed corpus for the paper's topic.
2. `mcp__home-still__catalog_list` / `mcp__home-still__catalog_recent` — browse library metadata for context or recent work.
3. `mcp__home-still__paper_search` — multi-database web search for gaps.
4. `mcp__home-still__paper_download` — ingest any new paper destined to be cited.
5. `mcp__home-still__markdown_read` / `catalog_read` — read and cite from the local copy.

### Citation verification (`citation-check` mode, `citation_compliance_agent`)

- For every in-text citation, resolve against `mcp__home-still__catalog_read` (by DOI or stem).
- For citations not in the local catalog, resolve via `mcp__home-still__paper_get` (multi-provider).
- Every citation in the final manuscript must carry a recorded `hs_stem` OR a `paper_get`-verified DOI. No exceptions — the `academic-pipeline` integrity gate enforces this.

### Style Calibration (intake)

- If the user provides DOIs or stems for past papers, pull them via `mcp__home-still__catalog_read` + `mcp__home-still__markdown_read` — no external PDFs needed.
- If the user pastes text directly, use it as-is (no tool call).

### Health preflight (full, lit-review, revision modes)

1. `mcp__home-still__system_status`
2. `mcp__home-still__distill_status` — confirm CUDA before running large literature sweeps.

### Failure modes

- If home-still MCP is unreachable: halt. Do not silently fall back to WebSearch-only — the Source Verification and Citation Compliance agents will fail their gates.
- If a cited paper is not yet indexed after `paper_download`: proceed with `markdown_read` using the returned stem; citation is still verifiable.

### Artifact placement

- Drafts, outlines, review letters, revision roadmaps → `~/research/<topic-slug>/` on the local machine.
- Do not write manuscript artifacts onto the home-still host.
