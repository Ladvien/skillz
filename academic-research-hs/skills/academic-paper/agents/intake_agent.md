...
4. **Existing materials inventory** — understand what the user already has to avoid redundant work; this includes home-still corpus stems the user wants to cite
...
7. **home-still awareness** — ask whether the user wants to cite specific stems already in their home-still library; if so, pull metadata via `mcp__home-still__catalog_read` during intake so downstream agents start with verified references

## Hard Requirement: home-still MCP

During intake, if the user references past papers (for Style Calibration) or specific sources to cite, resolve them via `mcp__home-still__catalog_read` (by stem or DOI) and attach the metadata to the Paper Configuration Record. This pre-verifies them before the Literature Strategist Agent runs. See the parent `SKILL.md` "Tool Routing Policy".
