...
# Academic Paper Reviewer v1.8.1 — Multi-Perspective Academic Paper Review Agent Team (home-still edition)
...
**hs-fork v1.8.1-hs.1** — When reviewers need to verify that cited sources exist or check for contradicting literature, they go through the home-still MCP server (see Tool Routing Policy below). Reviewers do not typically discover new papers — they verify what the author cited — so the policy focuses on catalog lookup and DOI resolution, not broad literature sweeps.

---

## Tool Routing Policy — home-still MCP (REQUIRED for citation verification)

Peer reviewers verify citations, not discover new literature. When a reviewer needs to check whether a reference exists, pull its metadata, or look for counter-evidence:

### Citation verification
1. `mcp__home-still__catalog_read` by DOI — if the paper is in the local library, read its metadata and (if needed) its markdown for context.
2. `mcp__home-still__paper_get` by DOI — multi-provider lookup for sources not in the catalog.
3. `mcp__home-still__distill_search` — when a reviewer wants to probe the local corpus for counter-evidence ("are there papers arguing the opposite?") or for missing foundational references ("should this paper have cited X?").

### Flagging fabricated references
- If a cited DOI fails both `catalog_read` and `paper_get`, flag it as `UNVERIFIED` in the review report.
- If a DOI resolves in `paper_get` but the title mismatch vs. the citation is high (Levenshtein < 0.70), flag as `DOI_MISMATCH`.
- These findings go into the reviewer's "Methodology" or "Integrity" section, not the content critique.

### What reviewers do NOT do

- Reviewers do NOT run broad `paper_search` sweeps. That is the author's (and the `academic-paper` / `deep-research` skills') job. A reviewer's job is to critique what's in front of them, not rebuild the bibliography.
- Reviewers do NOT call `paper_download` except when the Devil's Advocate wants to bring a specific counter-evidence source to the author's attention (rare, explicit).

### Failure modes
- If home-still MCP is unreachable: proceed with the review, but note in the report that citation verification was performed against author-provided DOIs only (no catalog cross-check). Flag this limitation.
